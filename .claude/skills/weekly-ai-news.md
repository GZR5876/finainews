...
Work through each of the four categories below in order. For each category:

1. Read the corresponding source file (references/sources_{category}.md).
2. Use WebSearch + WebFetch to find items published in the last 7 days from those sources.
...
4. Score each surviving item on three dimensions (1–10 each):
   - **Relevance** to a CFO of a global port operator
...
6. Keep top 5–8 items. Record each as:
   ```
   id: {category}_{NNN}
   headline: ...
...
4. **STOP.** Tell the user:
   ...
   > render when ready."
5. Wait for the user to say "render" or equivalent before proceeding.
